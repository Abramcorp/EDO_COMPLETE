...
def compute_tax_by_period(
    ...
    for period in ("q1", "half_year", "nine_months", "year"):
        ...
        tax = (income * Decimal(str(rate)) / Decimal("100")).quantize(Decimal("1"))
        raw_reduction = Decimal(str(contributions.get(period, 0)))
        reduction = min(raw_reduction, tax)  # can't reduce below 0
